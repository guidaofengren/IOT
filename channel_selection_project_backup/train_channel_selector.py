...
def parse_args():
    ...
    parser.add_argument("-output", default="channel_selector_result.json")
...
def main():
    ...
    final_test_X = final_test_X.to(args.device)
    final_test_y = final_test_y.to(args.device)
    ...
    model.eval()
    with torch.no_grad():
        logits, aux = model(final_test_X)
        test_acc = (logits.argmax(dim=1) == final_test_y).float().mean().item()
        channel_weights = aux["channel_weights"].detach().cpu().tolist()
    ...
    ckpt_path = os.path.join(os.path.dirname(__file__), f"channel_selector_s{args.subject_id}.pth.tar")
