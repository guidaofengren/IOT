"""
algos.py — Graph algorithms (Floyd-Warshall, edge input generation).
...
def gen_edge_input(max_hop, path, edge_feat, region):
    ...
    for i in range(n):
        for j in range(n):
            if i == j:
                continue
            p = [i] + get_all_edges(path_copy, i, j) + [j]
            num_path = len(p) - 1
            for k in range(num_path):
                flag = -1 if region[p[k]] == region[p[k + 1]] else -2
                first = min(p[k], p[k + 1])
                second = max(p[k], p[k + 1])
